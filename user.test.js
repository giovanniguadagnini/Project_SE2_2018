...
test('GET /users?access_token=validId with enrolledAfter & enrolledBefore param; should return 200 + all users in the system', async () => {
    expect.assertions(2);
    let response = await request(app).get('/users?access_token=' + validId).query({enrolledAfter: '1900', enrolledBefore: '2019'});
    expect(response.statusCode).toBe(200);
    expect(response.body).toBeDefined();
});
...
test('GET /users?access_token=validId with alpha sorting, enrolledAfter & enrolledBefore param; should return 200 + all users in the system', async () => {
    expect.assertions(2);
    let response = await request(app).get('/users?access_token=' + validId).query({sortUsrBy: 'alpha', enrolledAfter: '1900', enrolledBefore: '2019'});
    expect(response.statusCode).toBe(200);
    expect(response.body).toBeDefined();
});
